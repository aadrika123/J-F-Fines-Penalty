...
const SideBar = (props) => {
  ...
  return (
    <>
      ...
      {/* 👉 ======Main Section========== 👈 */}
      <header className={(toggleBar ? open1 : close1) + ` relative select-none transition-all duration-200 h-full text-${tcolor}-800 pt-2 border-r border-${tcolor}-200`}>
                  ...
                  <ul id="side-menu" class="w-full float-none flex flex-col">
                    {
                      props?.menu?.map((item) => <>
                        <li className='relative cursor-pointer mb-1' onClick={() => { (window.innerWidth <= 763 && item?.children?.length == 0) && settoggleBar(!toggleBar) }}>
                          ...
                          {(item?.children?.length > 0 && (dropName == item?.name)) && <ul class={`block rounded rounded-t-none top-full py-0.5 ltr:text-left rtl:text-right mb-4 bg-${mcolor}-200`} >
                            {
                              item?.children?.map((elem) => <>
                                <li class="relative cursor-pointer mt-1" onClick={() => { window.innerWidth <= 763 && settoggleBar(!toggleBar) }}>
                                  <NavLink to={elem?.path} className={({ isActive }) => (isActive ? `bg-${mcolor}-600 text-${tcolor}-100 `  : " ") + `${dropMenuBtn} ` + 'flex gap-3 items-center'}><span><MdOutlineSpaceDashboard /></span> <span className=''>{elem?.name}</span></NavLink>
                                </li>
                              </>)
                            }
                          </ul>}
